        ...
        for host in quaggaHosts:
            ...
            if host.name == 'r02':
                ...
                cli01r02 = self.addHost( name='cli01r02',
                                            ...
                                            inMountNamespace=True,
                                            inPIDNamespace=True)
                self.addNodeService(node='cli01r02',
                                    service=quaggaSvc,
...
def addInterfacesForNetwork(net):
    ...
    for host in hosts:
        ...
        if host.name == 'r02':
            ...
            host.cmd('sudo ifconfig r02-eth1 11.0.0.1 netmask 255.255.255.0 up')
        ...
        if host.name == 'r03':
            ...
            host.cmd('sudo ifconfig r03-eth1 10.0.0.1 netmask 255.255.255.0 up')
            host.cmd('sysctl -w net.ipv4.conf.r03-eth0.rp_filter=0')
            host.cmd('sysctl -w net.ipv4.conf.r03-eth1.rp_filter=0')
        if host.name == 'r04':
            host.cmd('sudo ifconfig lo:140 140.0.0.1 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig lo:141 140.0.0.2 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig lo:142 140.0.0.200 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig r04-eth1 10.1.0.1 netmask 255.255.255.0 up')
            host.cmd('sysctl -w net.ipv4.conf.r04-eth0.rp_filter=0')
            host.cmd('sysctl -w net.ipv4.conf.r04-eth1.rp_filter=0')
        if host.name == 'cli01r02':
            host.cmd('sudo ifconfig lo:150 150.0.0.1 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig lo:151 150.0.0.2 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig lo:152 150.0.0.200 netmask 255.255.255.0 up')
            host.cmd('sudo ifconfig cli01r02-eth1 10.1.0.2 netmask 255.255.255.0 up')
        if host.name == 'a01':
