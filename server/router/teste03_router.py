...
try:
    ...
    aux = res.send("agent<->server", 'agent', 'neighbor 172.0.0.3 withdraw route 200.0.0.0/23 next-hop 172.0.0.1')
    aux = res.send("agent<->server", 'agent', 'neighbor 172.0.0.3 announce route 200.0.0.0/23 next-hop 172.0.0.1 as-path [ 100 150 ]')

    for i in range(0,16):
        for j in range(0,256):
            msg = 'neighbor 172.0.0.3 announce route 100.%s.%s.0/24 next-hop 172.0.0.1 as-path [ 100 150 ]' % (i,j)
